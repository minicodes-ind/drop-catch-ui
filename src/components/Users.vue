    ...
    <header>
        ...
        <div class="card">
            <DataTable :value="usersData" :paginator="true" :rows="5" 
                    ...
                    :globalFilterFields="['nickname','url','initialPrice']"
                    v-model:filters="filters" filterDisplay="menu"
                    :rowHover="true"
                    >
                    <template #header>
                    <div class="flex justify-content-center align-items-center">
                        <h5 class="m-0">Search Alerts &nbsp;</h5>
                        <span class="p-input-icon-left">
                            <i class="pi pi-search" />
                            <InputText v-model="filters['global'].value" placeholder="url, nickname or price" />
                        </span>
                    </div>
                </template>
                <!-- <template #empty>
                ...
                <Column :exportable="false" style="min-width:8rem" >
                    <template #body="usersData">
                        <Button @click="confirmPosition('bottom', usersData.data.url, usersData.data.nickname)" 
    ...
    .card{
        padding: 6px;
        border-radius: 5px;
        box-shadow: -3px 3px rgb(212, 233, 252);
